...
앞서 말했듯이 이 구현의 네트워크에는 **중앙 노드**가 존재합니다. 중앙 노드는 **localhost:3000**으로 하드코딩 되어 있습니다. 네트워크를 참여하고자 하는 노드는 이 중앙 노드를 통해 블록체인을 받아오거나 네트워크에 속한 참여자들을 받아올 수 있습니다. 그림으로 나타내면 아래와 같습니다.

![Each node has own blockchain and wallet](../.gitbook/assets/image%20%28100%29.png)

다음은 우리가 테스트해볼 비트코인의 가장 중요한 플로우인 '코인 거래' 시나리오입니다.

1. 중앙 노드가 블록체인을 생성한다.
2. 다른 노드들은 중앙 노드에 연결하여 블록체인을 다운로드 받는다.
3. 채굴자 노드들도 중앙 노드에 연결하여 블록체인을 다운로드 받는다.
4. 어떠한 노드가 트랜잭션을 생성한다.
5. 중앙노드와 채굴자 노드는 트랜잭션을 받아 메모리풀에 저장한다.
6. 메모리풀에 충분한 양의 트랜잭션이 쌓이면 채굴자 노드는 새로운 블록의 채굴을 시작한다.
7. 새로운 블록이 채굴되면, 중앙 노드에 전송한다.
8. 중앙 노드는 다른 모든 노드에게 블록을 전송하여 동기화한다.
