...
이번 파트는 네트워크에 속한 다른 참여자들에게 요청을 보내거나 요청을 받았을 때 처리하는 전체적인 틀에 대해 구현을 할 것이고 각각 함수들의 세부 구현은 다음 파트에서 진행된다.

## blockchain/chain\_iter.go

BlockchainIterator를 따로 파일을 분리한다. 원래는 `blockchain.go` 파일에 있던 코드입니.
